...
本项目的主美术资产总图（用户原创生成，8K，`assets/bunker/asset_atlas_8k.png`）——涵盖角色（守军 / 感染者）、武器、道具、房间内饰、结构件（墙 / 门 / 地板）、招牌霓虹、UI 图标与特效，作为后续美术统一替换的唯一来源。

![Cyber Bunker Defense — Asset Atlas](assets/bunker/asset_atlas_preview.png)

> **下一步：整理（切片）这张总图。** 建立精确的切片清单（每个精灵名 → 像素框 / 帧规格），按区块切成带透明通道的命名精灵（角色动画帧、房间、道具、结构件、招牌、图标、特效），再分批替换进游戏（角色 → 场景 → 道具/特效 → 接回玩法与验收）。每批「切一批 → 校对 QA → 接代码」。
